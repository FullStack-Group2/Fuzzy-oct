...
export const CustomerProfile: React.FC<CustomerProfileProps> = () => {
  ...
  const fetchCustomer = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/customers/${user.id}`,
        {
          headers: {
  ...
  // Load customer data on component mount from AuthProvider or fetch if needed
  React.useEffect(() => {
    if (user && user.role === 'CUSTOMER') {
      // If we don't have complete customer data from AuthProvider, fetch it
